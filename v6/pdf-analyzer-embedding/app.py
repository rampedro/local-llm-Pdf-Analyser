...
# Function to extract word-level embeddings using GloVe
def get_word_level_embedding(text):
    """
    Word-level embeddings using GloVe or Word2Vec.
    Example using GloVe embeddings.
    """
    # Using pre-trained GloVe embeddings as an example
...
# Function to extract sentence-level embeddings using SentenceTransformers
def get_sentence_level_embedding(text):
    """
    Sentence-level embeddings using SentenceTransformer (e.g., MiniLM).
    """
    model = SentenceTransformer('paraphrase-MiniLM-L6-v2')
    embedding = model.encode(text)
    return embedding

# Function to extract paragraph-level embeddings using SentenceTransformer
def get_paragraph_level_embedding(text):
    """
    Paragraph-level embeddings using SentenceTransformer (e.g., all-MiniLM).
    """
...
def extract_categories_from_text(text):
    ...
    try:
        # Prepare the prompt for category extraction
        prompt = CATEGORY_EXTRACTION_PROMPT.format(context_str=text)

        # Get the embeddings at word, sentence, and paragraph levels
        word_embedding = get_word_level_embedding(text)
        sentence_embedding = get_sentence_level_embedding(text)
        paragraph_embedding = get_paragraph_level_embedding(text)

        # Log embeddings for debugging
        logging.info(f"Word-level embedding: {word_embedding.shape}")
        logging.info(f"Sentence-level embedding: {sentence_embedding.shape}")
        logging.info(f"Paragraph-level embedding: {paragraph_embedding.shape}")

        # Call Ollama model to extract categories based on the context provided in the prompt
