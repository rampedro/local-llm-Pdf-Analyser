...
# Function to extract word-level embeddings using SentenceTransformer
def get_word_level_embedding(text):
    """
    Word-level embeddings using SentenceTransformers (works well for short texts).
    """
    model = SentenceTransformer('paraphrase-MiniLM-L6-v2')
    embedding = model.encode(text)
    return embedding

# Function to extract sentence-level embeddings
def get_sentence_level_embedding(text):
    """
    Sentence-level embeddings using SentenceTransformer.
    """
    model = SentenceTransformer('paraphrase-MiniLM-L6-v2')
    embedding = model.encode(text)
    return embedding

# Function to extract paragraph-level embeddings
def get_paragraph_level_embedding(text):
    """
    Paragraph-level embeddings using SentenceTransformer.
    """
...
def extract_categories_from_text(text):
    ...
    try:
        # Generate embeddings for word, sentence, and paragraph levels
        ...
        logging.info(f"Word-level embedding shape: {word_embedding.shape}")
        logging.info(f"Sentence-level embedding shape: {sentence_embedding.shape}")
        logging.info(f"Paragraph-level embedding shape: {paragraph_embedding.shape}")

        # Prepare the prompt for category extraction
        prompt = CATEGORY_EXTRACTION_PROMPT.format(context_str=text)

        # Call Ollama model to extract categories
